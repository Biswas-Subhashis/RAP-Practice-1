...
*** Declaration
    TYPES: BEGIN OF ty_buffer_ord.
             INCLUDE TYPE ztsord_vendor_sb AS data.
...
*** Get last saved sales order
        SELECT MAX( sord )
          FROM ztsord_vendor_sb
          INTO @DATA(lv_max_sord).
        ...
        INSERT VALUE #(
          %cid = <lfs_entities>-%cid
          sord = lv_max_sord ) INTO TABLE mapped-vendor.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
...
*** Get already saved data
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<lfs_keys>).
      SELECT *
        FROM ztsord_vendor_sb
       WHERE sord = @<lfs_keys>-sord
        INTO TABLE @DATA(lt_result_tmp) .

      result = VALUE #(
                 FOR ls_result_tmp IN lt_result_tmp
...
*** Enable/Disable rows based on Overall Status
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<lfs_keys>).
      SELECT SINGLE *
        FROM ztsord_vendor_sb
       WHERE sord = @<lfs_keys>-sord
        INTO @DATA(ls_result_tmp) .

      ls_result-%key                         = <lfs_keys>-sord.
      ls_result-%features-%action-set_status = COND #(
                                                 WHEN ls_result_tmp-overallstatus = 'R' THEN
...
CLASS lsc_zi_sord_vendors_sb DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zi_sord_vendors_sb IMPLEMENTATION.
